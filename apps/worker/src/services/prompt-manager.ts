  ...
  try {
    ...
    if (authentication.credentials) {
      ...
      if (authentication.credentials.totp_secret) {
        userInstructions = userInstructions.replace(/\$totp/g, `generated TOTP code using the 'generate-totp' tool`);
      }
    }

    loginInstructions = loginInstructions.replace(/{{user_instructions}}/g, userInstructions);

    // 5. Removed TOTP secret placeholder replacement to prevent hardcoded secrets
